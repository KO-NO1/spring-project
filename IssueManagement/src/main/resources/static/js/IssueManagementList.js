...
$(function() {
	// テーブルの行をクリックしたときの処理
	$('#dtoListInfo tbody tr').on('click', function() {
		// すべての行の選択状態を解除
		$('#dtoListInfo tbody tr').removeClass('table-row-active');
...
function editSelectedLoginId(row) {
	row.find('td').each(function() {
		...
		if (columnId.startsWith('controlNumber')) {
			$('#selectedControlNumber').val($(this).text());
			return false;
		}
	});
}